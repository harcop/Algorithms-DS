...
fn main() {
    println!("{:?}", insertion_sort_list(None));
}

#[cfg(test)]
mod tests {
    use super::{insertion_sort_list, ListNode};
    ...
    fn example_one() {
        assert_eq!(
            to_vec(insertion_sort_list(list_from(&[4, 2, 1, 3]))),
            vec![1, 2, 3, 4]
        );
    }
    ...
    fn example_two() {
        assert_eq!(
            to_vec(insertion_sort_list(list_from(&[-1, 5, 3, 4, 0]))),
            vec![-1, 0, 3, 4, 5]
        );
    }
}
