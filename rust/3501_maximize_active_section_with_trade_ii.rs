...
fn max_active_sections_after_trade(s: String, queries: Vec<Vec<i32>>) -> Vec<i32> {
    ...
    let mut sparse_table = vec![0; adjacent_pair_count * sparse_level_count.max(1)];
    for pair_index in 0..adjacent_pair_count {
        sparse_table[pair_index] = (zero_blocks[pair_index].1 + zero_blocks[pair_index + 1].1) as i32;
    }
    ...
    let max_pair_sum = |left_pair: usize, right_pair: usize| -> i32 {
        if adjacent_pair_count == 0 || left_pair >= adjacent_pair_count {
            return 0;
        }
        ...
        let level = usize::BITS as usize - (right_pair - left_pair + 1).leading_zeros() as usize - 1;
        let span = 1usize << level;
    ...
    queries
        ...
        .map(|query| {
            ...
            if bytes[left] == b'0' && bytes[right] == b'0' && left_block_index + 1 == right_block_index
            {
                best_total = best_total.max(total_ones + left_zero_count + right_zero_count);
            } else if first_internal_pair >= 0 && first_internal_pair <= last_internal_pair {
                best_total = best_total.max(
                    total_ones
                        + max_pair_sum(first_internal_pair as usize, last_internal_pair as usize),
                );
            }
            if bytes[left] == b'0' && left_block_index + 1 <= last_full_zero_block {
                best_total = best_total.max(
                    total_ones + left_zero_count + zero_blocks[(left_block_index + 1) as usize].1 as i32,
                );
            }
...
fn main() {
    println!(
        "{:?}",
        max_active_sections_after_trade("01".into(), vec![vec![0, 1]])
    );
}
...
mod tests {
    ...
    fn example2() {
        assert_eq!(
            max_active_sections_after_trade(
                "0100".into(),
    ...
    fn example3() {
        assert_eq!(
            max_active_sections_after_trade("1000100".into(), vec![vec![1, 5], vec![0, 6], vec![0, 4]]),
            vec![6, 7, 2]
        );
    }
    ...
    fn example4() {
        assert_eq!(
            max_active_sections_after_trade("01010".into(), vec![vec![0, 3], vec![1, 4], vec![1, 3]]),
            vec![4, 4, 2]
        );
    }
}
