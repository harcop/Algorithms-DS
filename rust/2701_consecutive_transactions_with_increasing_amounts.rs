/// LeetCode #2701 - Consecutive Transactions with Increasing Amounts (MySQL)
...
mod tests {
    ...
    #[test]
    fn finds_increasing_streaks() {
        let upper = SQL.to_uppercase();
        assert!(upper.contains("DATEDIFF"));
        assert!(upper.contains("HAVING"));
        assert!(upper.contains("CONSECUTIVE_START"));
        assert!(upper.contains("CONSECUTIVE_END"));
