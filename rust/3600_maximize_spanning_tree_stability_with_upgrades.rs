...
fn max_stability(n: i32, edges: Vec<Vec<i32>>, k: i32) -> i32 {
    ...
    let check = |lim: i32| {
        ...
        let mut rem = k;
        for e in &edges {
            if rem > 0 && e[3] == 0 && e[2] * 2 >= lim && uf.union(e[0], e[1]) {
                rem -= 1;
            }
        }
        uf.cnt == 1
...
fn main() {
    println!(
        "{}",
        max_stability(3, vec![vec![0, 1, 2, 1], vec![1, 2, 3, 0]], 1)
    );
}

#[cfg(test)]
mod tests {
    use super::max_stability;
