/// LeetCode #2494 - Merge Overlapping Events in the Same Hall (MySQL)
...
mod tests {
    ...
    #[test]
    fn merges_by_hall_and_group() {
        assert!(SQL.contains("HallEvents"));
        assert!(SQL.contains("group_id"));
        assert!(SQL.to_uppercase().contains("PARTITION BY"));
    }
}
