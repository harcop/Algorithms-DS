...
fn can_reach_corner(x_corner: i32, y_corner: i32, circles: Vec<Vec<i32>>) -> bool {
    let n = circles.len();
...
fn main() {
    println!("{}", can_reach_corner(3, 4, vec![vec![2, 1, 1]]));
}

#[cfg(test)]
mod tests {
    use super::can_reach_corner;

    #[test]
    fn example1() {
        assert!(can_reach_corner(3, 4, vec![vec![2, 1, 1]]));
    }

    #[test]
    fn example2() {
        assert!(!can_reach_corner(3, 3, vec![vec![1, 1, 2]]));
    }

    #[test]
    fn example3() {
        assert!(!can_reach_corner(3, 3, vec![vec![2, 1, 1], vec![1, 2, 1]]));
    }

    #[test]
    fn example4() {
        assert!(can_reach_corner(4, 4, vec![vec![5, 5, 1]]));
    }
}
